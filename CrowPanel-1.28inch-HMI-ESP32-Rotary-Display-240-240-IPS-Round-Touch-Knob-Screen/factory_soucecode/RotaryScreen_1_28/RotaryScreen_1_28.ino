...
#include "USB.h"                    //gemini
#include "USBHIDConsumerControl.h"  // gemini
...
USBHIDConsumerControl ConsumerControl;  //gemini
int lastStateCLK_G; // gemini
...
void setup() {
  ...
  // Initialize USB HID. --gemini
  ConsumerControl.begin();
  USB.begin();
  lastStateCLK_G = digitalRead(ENCODER_A_PIN);
//end gemini
...
int currentStateCLK = digitalRead(ENCODER_A_PIN);
  ...
  if (currentStateCLK != lastStateCLK_G && currentStateCLK == HIGH) {
  ...
  // Check if button was pressed (Mute)
  ...
  lastStateCLK_G = currentStateCLK;
  // delay(1); // Small delay for stability. - gemini
